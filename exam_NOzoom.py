...
tk.Label(left_frame, textvariable=question_text, wraplength=400, font=("Arial", 12)).pack(pady=10)
tk.Label(left_frame, textvariable=question_type_label, font=("Arial", 12)).pack(pady=5)
options_frame = tk.Frame(left_frame)
options_frame.pack()

# Timer display
timer_label = tk.Label(left_frame, text="已用时间: 0 秒", font=("Arial", 12))
timer_label.pack()
update_timer()

# Next question button and Submit button
tk.Button(left_frame, text="下一题", command=next_question, font=("Arial", 12)).pack(pady=10)
tk.Button(left_frame, text="提交", command=submit_exam, font=("Arial", 12)).pack(pady=10)

# Answer card display on the right
tk.Label(right_frame, text="答题卡", font=("Arial", 12)).pack()
answer_card_frame = tk.Text(right_frame, height=20, width=30, font=("Arial", 12))
